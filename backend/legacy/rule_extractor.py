...
class RuleExtractor:
    def __init__(self):
        ...
        with open(os.path.join(prompt_dir, "unused_extraction_prompt_definitions.txt"), "r", encoding="utf-8") as f:
            self.extraction_prompt_definitions = f.read()
            
        with open(os.path.join(prompt_dir, "unused_extraction_prompt_guidelines.txt"), "r", encoding="utf-8") as f:
            self.extraction_prompt_guidelines = f.read()

        # Phase 2: Tagging
        tagging_prompt_path = os.path.join(prompt_dir, "unused_tagging_prompt.txt")
        with open(tagging_prompt_path, "r", encoding="utf-8") as f:
            self.tagging_prompt = f.read()
            
        # Phase 3: Consolidation
        consolidation_prompt_path = os.path.join(prompt_dir, "unused_consolidation_prompt.txt")
        with open(consolidation_prompt_path, "r", encoding="utf-8") as f:
            self.consolidation_prompt = f.read()
            
        # Phase 4: Deduplication
        rule_dedup_prompt_path = os.path.join(prompt_dir, "unused_rule_dedup_prompt.txt")
